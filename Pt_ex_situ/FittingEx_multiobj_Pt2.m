...
switch target_NP
    case "Pt"
        ...
        lbub = [-100,0; ...lnqfqb
            ...
            265.077.^(-1/3),265.077.^(-1/3); ...sigma_s = 265.077.^(-1/3);
            0,inf; ...supersaturation ratio
            1,2 ... shape index
            ]';
        npdata = readmatrix("Pt_ex_situ.csv");
        nbin = [];
        smooth_window = [8, 5];
        IP_method = "makima";
end
...
%%
expdata = Preprocessing_ex_situ(npdata, "experiment_r3_PtEx_right.mat");
expdata = r2n_optim(expdata, lbub(1,6), IP_method, smooth_window);
%% Initial Setting
galbub = [-6,-1; ...lnqfqb
           -6,6; ...lnqeqb
            lbub(1, 3),10; ...alpha+4
...
function cost = Jn_loss(x, expdata, options)
    cost = Calculate_Cost_Ex_situ_fast2(x, expdata, options);
end
function cost = Var_loss(x, expdata, options)
    [~, var_theory] = Calculate_Mean_Var_Ex_situ2(x, expdata, options);
...
function cost = JnVar_loss(x, expdata, options)
    cost(1) = Calculate_Cost_Ex_situ_fast2(x, expdata, options);
    [~, var_theory] = Calculate_Mean_Var_Ex_situ2(x, expdata, options);
    var_exp = options.factor*expdata.smooth_varnumberdata;
    loss = options.loss_var;
    scale = options.scale_var;
