function [exp_mundata, muns_mn, muns_log] = Extract_chemical_potential_In_situ(xfit, expdata, options)
    ...
    for idx = 1:lent
        ...
        kna_mean = mean(kna);
        knd_mean = mean(knd);
        dtlnrhoct(idx) = (kna_mean*supsat_ - knd_mean - fnval(dmeanf,T(idx))) ...
                ./(smooth_meannumberdata(idx)-1); % Equation (S5-5)
    end
    ...
    for idx = 1:lent
        ...
        exp_mudata_table1(nval-minval+1,idx) = exp_mudata; 
    end
    ...
    % Chemical potential from kna and knd assuming that shape index = mean(shape_index)
