function cost = lossRegWeighted(prediction, experiment, loss_method, scale_method, weights)
    ...
    switch loss_method
        case "mse" % mean squared error
            cost = mean((prediction - experiment).^2.*weights);
        case "logmse" %log(mean squared error)
            cost = log(mean((prediction - experiment).^2.*weights));
        case "sse" % sum of squared error
            cost = sum((prediction - experiment).^2.*weights);
        case "logsse" % log(sum of squared error)
            cost = log(sum((prediction - experiment).^2.*weights));
        case "meanlogcosh" % mean of log cosh error
            cost = mean(log(cosh(prediction - experiment)).*weights);
        case "sumlogcosh" % sum of log cosh error
            cost = sum(log(cosh(prediction - experiment)).*weights);
        case "rmse" % relative mean squared error
            % experiment should always have positive value
            % do not scale for this loss function
            cost = mean((prediction - experiment).^2./experiment.^2.*weights);
        otherwise
