function Population = gacuGuess2_right(GenomeLength,FitnessFcn,options, expdata, other_opt, situ)
    ...
    pop_ = Population(initPopProvided+1:end,:);
    switch situ
        case "in"
            supsat = guess_supsat_In_situ(pop_, expdata, other_opt);
        case "ex2"
            supsat = guess_supsat_Ex_situ2(pop_, expdata, other_opt);
        otherwise
    end
    Population(initPopProvided+1:end,ind{7}) = supsat;
