function [exp_mundata, muns_mn, muns_log] = Extract_mun_Ex_situ_For_Figure5(xfit, expdata, options)
    ...
    for idx = 1:lent
        ...
        kna_mean = sum(kna.*pn);
        knd_mean = sum(knd.*pn);
        correction(idx) = (kna_mean*supsat_ - knd_mean - fnval(dmeanf,T(tidx))) ...
    ...
    for idx = 1:lent
        ...
        val = (1 + 1./(nval-1)).^alp_.*(qfqb_.^delta_Nf).*(qeqb_.^delta_Ne);  
        knd = 4.*pi.*sigma_s^2.*kar1inf_.*sf_.*(nval-1).^(2/3)./(1+kass_d1_.*(nval-1).^(1/3))./val; 
        exp_mudata = -0.5.*log((((p_time_dev_val + correction(idx).*pcum_val)./(knd.*pvalue) + 1).*ratiodata).^2) ...
            + log(supsat_); 
        exp_mudata(exp_mudata >= 2 | exp_mudata <= -0.3) = nan; 
